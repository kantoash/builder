...
import { SubAccountDetailsSchema } from "@/lib/schema";
...
export const SubAccountDetails: React.FC<Props> = ({
  details,
  agencyDetails,
  userId,
  userName,
  ...
  const Submit = async (values: z.infer<typeof SubAccountDetailsSchema>) => {
    try {
      ...
      setClose();
      router.replace(
        `${process.env.NEXT_PUBLIC_URL}/subaccount/${response.id}`
      );
    } catch (error) {
