...
import { v4 } from "uuid";
import { AgencyDetailsSchema } from "@/lib/schema";
...
export const AgencyDetails = ({ data }: AgencyDetailsProps) => {
  ...
  const Submit = async (values: z.infer<typeof AgencyDetailsSchema>) => {
    try {
      let newUserData;
      let custId;
      if (!data?.id) {
  ...
  const handleDeleteAgency = async () => {
    ...
    //challenge: to do discontinue the subscription
    try {
  ...
  const GoalSet = async (val: number) => {
    if (!data?.id) return;
    ...
    <AlertDialog>
      <Card className="w-full ">
        ...
        <CardContent>
          ...
          {data?.id && (
            <div className="flex flex-row items-center justify-between rounded-lg border border-destructive gap-4 p-3 mt-4">
              <div>
                <div>Danger Zone</div>
